...
function init(){
  ...
  annyang.addCallback('result', function(phrases) {
    document.getElementById('input_text').innerHTML = "I think the user said: " + phrases;
  });
}
}
  ...
  Object.keys(rows).forEach(function (row) { 
    Object.keys(columns).forEach(function (column) { 
      Object.keys(colors).forEach(function (color) { 
        var command = row + ' ' + column + ' ' + color;
        commands_hash[command] = function(){
          var id = rows[row] + columns[column];
          var selected_element = document.getElementById(id);
          selected_element.style.background = colors[color];
          selected_element.style.color = colors[color];
...
function setupShowHide(){
  commands_hash['hide guides'] = function(){document.body.classList.add('guides_hidden');};
  commands_hash['show guides'] = function(){document.body.classList.remove('guides_hidden');};
}